...
**Closure request history is now 1:M, not 1:1-overwritten (built and
live-verified — Phase 22).** A customer can request closure again on
the same account once a prior request is `REJECTED` or `CANCELLED`
(both revert `accounts.status` back to `ACTIVE`) — this used to
overwrite the single prior request's data (a known, accepted POC-scale
simplification this file's own schema comment used to flag); a new
`account_closure_requests` table (owned by `account/`, same as
`accounts` itself) now keeps one row per request instead, with its own
`workflow_run_id` (captured from `workflow.info().run_id` at
`CloseAccountWorkflow.run()` start — `workflow_id` alone can't
disambiguate a repeat request against the same account, since it's
deterministically reused every time). `accounts.status` itself is
unchanged — still current-state only. Live-verified end to end: a real
account requested → rejected → requested again → approved, confirmed
via `psql` to hold exactly two independent history rows (distinct
`closure_request_id`/`workflow_run_id`, correct separate
`decision_comment`/`decided_by`), and the product-type-elimination
payoff (above) still correctly reappears once the account reaches
`CLOSED` via the *second* request, not just the first. **One
pre-existing gap, not introduced or fixed by this redesign**:
`request_closure()`'s own race-safety against two near-simultaneous
requests for the same account is unchanged (relies on Temporal's own
`WorkflowAlreadyStartedError` for the actually-reachable race, not a
Postgres constraint). **Load the `account-closure` skill** for the
full 1:M design, including the closure-history UI surfaces on both the
customer detail page and the staff review dialog.
...
- **`account/activities.py`** — the concrete `CloseAccountWorkflow`
  ...
  (returns the `closure_request_id` it inserted or, on a retry, already
  found — Phase 22) and `persist_closure_decision(...)` (idempotency-
  guarded — a retry against a `closure_request_id` whose row has
  already moved past `PENDING` returns the account's current status
  without re-sending the closure-decision email; the guard reads that
  specific request row's own status now, not the account's). **See the
  `account-closure` skill** for the full design.
...
**Two Mayan identifiers, both kept (Phase 25, "Track Mayan's real UUID
alongside its integer id," built)**: `mayan_document_uuid TEXT UNIQUE`
holds Mayan's real `uuid` field, and a separate `mayan_id INTEGER
UNIQUE` holds its plain integer id. Confirmed live against the real
Mayan instance before adding the uuid column: Mayan's REST API is
entirely id-addressed for actual operations (file streaming, metadata
attach, upload) — `GET /documents/<uuid>/` returns a real `404`, and
...
one, so this app's own row is the only place that mapping lives. Both
values come from the exact same `mayan_client.create_document(...)`
POST response (confirmed live it already includes `uuid`, no second
Mayan call needed) — captured once, at write time, on every path that
creates a *new* Mayan document (`upload`,
`promote_government_id_to_customer_photo`'s copy-creation,
`generate_welcome_letter`, `upload_consent`'s create-first-version
branch). `upload_consent`'s true-versioning re-upload branch is the
one exception — no new Mayan document is created there, so it reuses
the *existing* Postgres row's own `mayan_document_uuid` rather than
...
- `service.upload_consent(applicant_identifier, account_id, customer_id,
  ...
  (same `account_document_id`, same `mayan_id`/`mayan_document_uuid`
  since Mayan versioned the existing document rather than creating a
  new one, new `filename`/`updated_at`). **(Phase 25, built)**: the
  re-upload branch's `mayan_document_uuid` comes from the *existing*
  Postgres row, not a fresh Mayan fetch — no new document, so no new
  uuid; the create-first-version branch captures a fresh one from
  `create_document(...)`'s own response, same as every other write path
  in this module.
- `service.preview_account_document(account_id, document_id) ->
  ...
  `document/db.py`'s by-`mayan_id` lookups instead of a live Mayan
  metadata fetch — the file itself still streams from Mayan either way,
  unchanged, since Postgres holds no file bytes. Still keyed on the
  integer id, not the uuid (Phase 25) — both routes' own URL parameter
  is still Mayan's integer id, unaffected by that phase.
...
own prefix plus a random 9-digit number, generated by a shared leaf
module, `idgen/` (see "Module dependency graph"):
...
in Phase 24, not present in any earlier module's `db.py`, doubled in
Phase 25**: the three new tables also carry two further uniqueness
concerns, `mayan_id` and `mayan_document_uuid` — but neither column is
ever generated by this app (both are Mayan's own values, handed back
from a `create_document` call), so there's no collision-retry loop for
either; a real collision on either would mean Mayan itself somehow
reused an id or a uuid, not something this app's own `idgen` call could
ever cause.
